...
void test_com()
{
  ...
  // TX and RX strings 11 for joystick x, 4 for joystick y, 0-3 for servo, 5 6 7 accelerometer,
    ...
    do {
        ...
        else if (user_Input == 2)
        {
            while (true)
            {
                ...
                if (!cc.get_data(cc.DIGITAL, SW1_CHANNEL, button_SW1))
                {
                    Sleep(50);
                    continue;
                }
        ...
        else if (user_Input == 4)
        {
            ...
            while (true)
            {
                ...
                for (int position_Input = 0; position_Input <= 180; position_Input = position_Input + 5)
                {
                    cc.set_data(cc.SERVO, SERVO_CHANNEL, position_Input);
                    cc.get_data(cc.SERVO, SERVO_CHANNEL, position_Coord);
                    channel_Num = cc.get_last_channel();
                    cout << "SERVO TEST: CH" << channel_Num << " = " << position_Coord << endl;

                    if (position_Input == 180)
                    {
                        for (int down = position_Input; position_Input >= 0; position_Input = position_Input - 5)
                        {
                            cc.set_data(cc.SERVO, SERVO_CHANNEL, position_Input);
                            cc.get_data(cc.SERVO, SERVO_CHANNEL, position_Coord);
                            channel_Num = cc.get_last_channel();
